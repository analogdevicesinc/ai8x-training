...
class LinearQuantAI84Mode(Enum):
    SYMMETRIC = 1
    ASYMMETRIC_UNSIGNED = 2
    ASYMMETRIC_SIGNED = 3


class ClipModeAI84(Enum):
    NONE = 0
    AVG = 1
    N_STD = 2


def quantize_clamp(input, clamp_min, clamp_max, inplace=False):
    if inplace:
        input.round_().clamp_(clamp_min, clamp_max)
        return input
...
def verify_quant_mode(mode):
    return _verify_enum_value(mode, LinearQuantAI84Mode)


def verify_clip_mode(mode):
    return _verify_enum_value(mode, ClipModeAI84)


def _get_saturation_fn(quant_mode, clip_mode, num_stds):
    if quant_mode == LinearQuantAI84Mode.SYMMETRIC:
        fns = {ClipModeAI84.NONE: get_tensor_max_abs,
               ClipModeAI84.AVG: get_tensor_avg_max_abs,
               ClipModeAI84.N_STD: partial(get_tensor_mean_n_stds_max_abs, n_stds=num_stds)}
    else:  # Asymmetric mode
        fns = {ClipModeAI84.NONE: get_tensor_min_max,
               ClipModeAI84.AVG: get_tensor_avg_min_max,
               ClipModeAI84.N_STD: partial(get_tensor_mean_n_stds_min_max, n_stds=num_stds)}
...
def _get_quant_params_from_tensor(tensor, num_bits, mode, clip=ClipModeAI84.NONE, per_channel=False, num_stds=None,
    ...
    dim = 0 if clip == ClipModeAI84.AVG or per_channel else None
...
def _get_quant_params_from_stats_dict(stats, num_bits, mode, clip=ClipModeAI84.NONE, num_stds=None,
    ...
    prefix = 'avg_' if clip == ClipModeAI84.AVG else ''
...
def add_post_train_quant_ai84_args(argparser):
    str_to_quant_mode_map = {'sym': LinearQuantAI84Mode.SYMMETRIC,
                             'asym_s': LinearQuantAI84Mode.ASYMMETRIC_SIGNED,
                             'asym_u': LinearQuantAI84Mode.ASYMMETRIC_UNSIGNED}

    str_to_clip_mode_map = {'none': ClipModeAI84.NONE, 'avg': ClipModeAI84.AVG, 'n_std': ClipModeAI84.N_STD}
    ...
    group.add_argument('--qe-config-file', type=str, metavar='PATH',
                       help='Path to YAML file containing configuration for PostTrainLinearQuantizerAI84 (if present, '
                            'all other --qe* arguments are ignored)')


class RangeLinearQuantAI84Wrapper(nn.Module):
    """
    ...
    def __init__(self, wrapped_module, num_bits_acts, num_bits_accum=32, mode=LinearQuantAI84Mode.SYMMETRIC,
        ...
        signed = mode != LinearQuantAI84Mode.ASYMMETRIC_UNSIGNED
...
class RangeLinearQuantAI84ParamLayerWrapper(RangeLinearQuantAI84Wrapper):
    """
    ...
    def quantized_forward(self, input_q):
        ...
        if self.mode != LinearQuantAI84Mode.SYMMETRIC:
...
class RangeLinearAI84EmbeddingWrapper(nn.Module):
    def __init__(self, wrapped_module, num_bits, mode=LinearQuantAI84Mode.SYMMETRIC, stats=None):
        ...
        super(RangeLinearAI84EmbeddingWrapper, self).__init__()
...
class PostTrainLinearQuantizerAI84(Quantizer):
    """
    ...
    def __init__(self, model, bits_activations=8, bits_parameters=8, bits_accum=32,
        ...
        if clip_acts == ClipModeAI84.N_STD and clip_n_stds is None:
        ...
        self.replacement_factory[distiller.modules.EltwiseAdd] = partial(
            replace_non_param_layer, RangeLinearQuantAI84EltwiseAddWrapper)
        self.replacement_factory[distiller.modules.EltwiseMult] = partial(
            replace_non_param_layer, RangeLinearQuantAI84EltwiseMultWrapper)
        self.replacement_factory[nn.Embedding] = replace_embedding

    @classmethod
    def from_args(cls, model, args):
        """
        Returns an instance of PostTrainLinearQuantizerAI84 based on the set command-line arguments that are
        given by add_post_train_quant_args()
        """
        if args.qe_config_file:
            return config_component_from_file_by_class(model, args.qe_config_file,
                                                       'PostTrainLinearQuantizerAI84')
        else:
            return cls(model,
...
class FakeLinearQuantizationAI84(nn.Module):
    def __init__(self, num_bits=8, mode=LinearQuantAI84Mode.SYMMETRIC, ema_decay=0.999, dequantize=True, inplace=False):
        super(FakeLinearQuantizationAI84, self).__init__()
    ...
    def forward(self, input):
        ...
        if self.mode == LinearQuantAI84Mode.SYMMETRIC:
            ...
            signed = self.mode == LinearQuantAI84Mode.ASYMMETRIC_SIGNED
            if self.training:
...
class FakeQuantizationAI84Wrapper(nn.Module):
    def __init__(self, wrapped_module, num_bits, quant_mode, ema_decay):
        super(FakeQuantizationAI84Wrapper, self).__init__()
        self.wrapped_module = wrapped_module
        self.fake_q = FakeLinearQuantizationAI84(num_bits, quant_mode, ema_decay, dequantize=True,
...
class QuantAwareTrainRangeLinearQuantizerAI84(Quantizer):
    def __init__(self, model, optimizer=None, bits_activations=32, bits_weights=32, bits_bias=32,
                 overrides=None, mode=LinearQuantAI84Mode.SYMMETRIC, ema_decay=0.999, per_channel_wts=False,
                 quantize_inputs=True, num_bits_inputs=None):
        super(QuantAwareTrainRangeLinearQuantizerAI84, self).__init__(model, optimizer=optimizer,
        ...
        if isinstance(model, nn.DataParallel) and len(model.device_ids) > 1:
            raise RuntimeError('QuantAwareTrainRangeLinearQuantizerAI84 currently does not support running with '
                               'multiple GPUs')
        ...
        def activation_replace_fn(module, name, qbits_map):
            ...
            if bits_acts is None:
                return module
            return FakeQuantizationAI84Wrapper(module, bits_acts, mode, ema_decay)
    ...
    def _prepare_model_impl(self):
        super(QuantAwareTrainRangeLinearQuantizerAI84, self)._prepare_model_impl()
        ...
        if self.quantize_inputs:
            ...
            m.inputs_quant = FakeLinearQuantizationAI84(self.num_bits_inputs, self.mode, self.decay,
