...
def _get_args():
    ...
    parser.add_argument(
        '--scale', default=1, dest='scl', type=float, help='scale factor')
    parser.add_argument(
        '--bits', default=8, dest='bits', type=int, help='number of bits')
...
def _make_node(nodes, op, name, inputs, **kwargs):
    node = helper.make_node(
        op, inputs, [_port_name(name)], name=name, **kwargs)
    nodes.append(node)
...
def _compose_quantitize(nodes, weights, zp, scale, name):
    ...
    weights.append(
        numpy_helper.from_array(np.array(zp, dtype=np.float32), name_zp))
    weights.append(
        numpy_helper.from_array(np.array(scale, dtype=np.float32), name_scale))

    # insert ops to dequantitize
    _make_node(nodes, "Cast", name_cast, [name],
               to=onnx_pb.TensorProto.FLOAT)  # pylint: disable=no-member
...
def quantitize_graph(g, scl, verbose=False):
    """Quantitize graph."""
    new_weights = []
    quantitized_weights = []
    nodes = []
    ...
    for i, w in enumerate(g.initializer):
        ...
        if w.data_type != onnx_pb.TensorProto.FLOAT:  # pylint: disable=no-member
            continue
...
def main():
    """main"""
    global RANGE  # pylint: disable=global-statement
    args = _get_args()
    ...
    print(RANGE)
    # quantitize weights
    g = quantitize_graph(model_proto.graph, args.scl, args.verbose)

    # write quantitized graph
    with open(args.output, "wb") as f:
        # create model proto
        model_proto_out = helper.make_model(
            g,
