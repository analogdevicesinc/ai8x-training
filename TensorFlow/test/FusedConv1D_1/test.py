...
# Init input samples
test_input = np.random.normal(0, 0.5, size=(17, 7))

print(test_input.shape)
test_input = clamp(np.floor(test_input*128 + 0.5))/128.0
print(test_input.shape)
test_input = np.reshape(test_input, (1, 17, 7))
...
input_layer = tf.keras.Input(shape=(17, 7))
reshape = tf.keras.layers.Reshape(target_shape=(17, 7))(input_layer)
...
# Save input
np.save(os.path.join(logdir, 'input_sample_17x7x1.npy'), np.array(saved_input, dtype=np.int32))
