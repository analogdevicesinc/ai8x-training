...
def KWS_Custom3_get_datasets(data, load_train=True, load_test=True,quantized=True):
    """
    Load a custom KWS dataset with 3 user-defined classes located in KWS/raw/class_name/.

    Dataset must already be placed in the correct structure. No download is attempted.
    """

    (data_dir, args) = data

    # Replace these with your actual folder/class names
    custom_classes = ['class1', 'class2', 'class3']  # <-- customize this line

    if quantized:
        transform = transforms.Compose([
            ai8x.normalize(args=args)
        ])
        quantization_scheme = {'compand': False, 'mu': 255}
        augmentation = {'aug_num': 2, 'shift': {'min': -0.1, 'max': 0.1},
    ...
    if load_train:
        train_dataset = KWS(root=data_dir, classes=custom_classes, d_type='train',
                            t_type='keyword', transform=transform,
                            quantization_scheme=quantization_scheme,
                            augmentation=augmentation, download=False)
    else:
        train_dataset = None

    if load_test:
        test_dataset = KWS(root=data_dir, classes=custom_classes, d_type='test',
                           t_type='keyword', transform=transform,
                           quantization_scheme=quantization_scheme,
                           augmentation=augmentation, download=False)

        if hasattr(args, 'truncate_testset') and args.truncate_testset:
            test_dataset.data = test_dataset.data[:1]
    else:
        test_dataset = None
...
datasets = [
    ...
    {
       'name': 'VPR_3',  # 20 keywords + _unknown_
        'input': (128, 128),
        'output':('class1', 'class2', 'class3'),
        'weight': (1, 1, 1),
        'loader': KWS_Custom3_get_datasets,
    },
    {
